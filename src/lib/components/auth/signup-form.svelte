...
<form class={cn("flex flex-col gap-6", className)} method="POST" action="?/default" {...restProps}>
	<Field.Group>
		...
		{#if formResult?.message}
			<p class="text-center text-sm text-destructive" role="alert">{formResult.message}</p>
		{/if}
		...
		<Field.Field>
			...
			<Field.Description>Must be at least 8 characters with uppercase, lowercase, number and special character.</Field.Description>
		</Field.Field>
		<Field.Field>
		...
		<Field.Field>
			<Button variant="outline" type="submit" formaction="?/github">
